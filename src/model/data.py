...
class ChordVocabulary:
    ...
    def build_vocab(self, chord_sequences: List[str], genres: List[str] = None):
        ...
        if genres:
            ...
            # Add Genres as unique tokens
            for genre in unique_genres:
                token_name = f"[GENRE_{genre.upper()}]"
                self.token2idx[token_name] = len(self.token2idx)
            self.token2idx["[GENRE_UNKNOWN]"] = len(self.token2idx)
...
class ChordDataModule(pl.LightningDataModule):
    ...
    def _process_dataset(self, dataset):
        ...
        for example in dataset:
            ...
            genre_token = self.vocab.token2idx.get(f"[GENRE_{genre.upper()}]", 
                                                self.vocab.token2idx["[GENRE_UNKNOWN]"])

            # Prepend the genre prefix BEFORE BOS
            token_ids = [genre_token] + self.vocab.encode(example['chords'])
